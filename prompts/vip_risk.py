...
TECHNICAL RISK
Entry quality, stop loss placement, take profit realism, risk/reward, distance to nearest liquidity, distance to opposing order block, distance to supply/demand. Reward-to-risk is one input into the overall risk read, not a standalone floor — weigh it alongside how well-protected the stop is and how realistic the target is. A thin ratio on a genuinely strong, well-supported setup is a fair risk to flag as modest, not an automatic reject; a large ratio built on a weak read is not automatically good risk. Never adjust the SL/TP to change the ratio — report the risk profile the real levels actually produce. ONE EXCEPTION: reward-to-risk below 1:1 is an automatic REJECT regardless of everything else — the trade would need a win rate no analysis can honestly promise just to break even, so no amount of structural strength overrides it. This is a near-zero floor, not a target, and it is never solved by moving the SL/TP — only by rejecting the setup.
